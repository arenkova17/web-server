...
from starlette.middleware.sessions import SessionMiddleware   #импорт middleware (промежуточного программного обеспечения) - нужен для того чтобы сервер запомнил юзер один раз и при каждом его запросе не кидал в базу вопрос, что это за юзер и можно ли ему доверять
from fastapi import Request, HTTPException
from starlette.middleware.base import BaseHTTPMiddleware
from starlette.responses import RedirectResponsfrom fastapi import FastAPI, Request
...
app.add_middleware(
    ...
    max_age=None,  #время работы сессии - пока не закроется браузер
...
if __name__ == '__main__':
    uvicorn.run("main:app", host="127.0.0.1", port=8000)e
...
# функция перехода на старницу договора когда нашли 1 договор через кнопку найти договор
@app.get("/search", response_class=HTMLResponse)
def search_page(numberdog: str = "", numberkontr: str = ""):
    results = search_dog(numberdog, numberkontr)
    if len(results) == 1:
...
def contract_page(contract_id: int, from_page: int = 1):
    ...
    payments = get_dog_payments(contract_id)
...
@app.post("/api/contract/{contract_id}/update")
async def update_contract_api(request: Request, contract_id: int):
#собирает параметры перед нажатием кнопки сохранить
    try:
        data = await request.json()
        konk_value = data.get('konk', 0)
